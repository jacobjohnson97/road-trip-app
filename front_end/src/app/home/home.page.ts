...
export class HomePage implements AfterViewInit {
  ...
  saveTrip() {
    ...
    if (!this.loggedIn) {
      ...
      this.http.post('https://34.70.173.192:3000/api/trip', body, httpOptions).subscribe((response) => {
  ...
  validateLogin() {
    if (localStorage.getItem('sectok')) {
      ...
      this.http.get('https://34.70.173.192:3000/api/token', httpOptions).subscribe((response) => {
        this.loggedIn = true;
      }, (err) => {
        this.loggedIn = false;
        localStorage.removeItem('sectok')
